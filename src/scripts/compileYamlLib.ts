...
import {getRecordValues, runningInJest} from '../client/utils';
...
export function getFilesToCache(finalObj: ReturnedFinalConfig): string[] {
    ...
    for (const appName in finalObj.appConfigs) {
        ...
        for (const rawDBConfig of getRecordValues(dbConfigs)) {
...
function constArrInner(constArr: Array<string>) {
    return constArr.map((tok) => `"${tok}"`).join(", ");
}
...
export function getTypeScriptHelperDefinitionsFromConfig(rfc: ReturnedFinalConfig): {filename: string, contents: string, after: () => Promise<void>}[] {
    ...
    const defaultDialectConfig = defaultLangConfig.dialects[DEFINITIONS_DEFAULT_LANG];
    const i18nTokens = defaultDialectConfig?.tokens;
    if (i18nTokens === undefined || Object.keys(i18nTokens).length < 1) {
        throw new Error(`Default language "${DEFINITIONS_DEFAULT_LANG} does not have tokens defined!"`);
    }

    const defaultFontConfig = rfc.default.configs.fontConfig.config;


    const knownI18NTokens = Object.keys(i18nTokens);
    const knownDialectIDs = Object.keys(defaultLangConfig.dialects);
    const knownFontIDs = Object.keys(defaultFontConfig.fonts);
    const knownFontGroupIDs = Object.keys(defaultFontConfig.fonts);

    const knownI18NTokensAsConstArrayInner = constArrInner(knownI18NTokens);
    const knownDialectIDsAsConstArrayInner = constArrInner(knownDialectIDs);
    const knownFontIDsAsConstArrayInner = constArrInner(knownFontIDs);
    const knownFontGroupIDsAsConstArrayInner = constArrInner(knownFontGroupIDs);
...
const defaultDialect = "${DEFINITIONS_DEFAULT_LANG}" as const;
const i18nTokenIDs = [${knownI18NTokensAsConstArrayInner}] as const;
const knownDialectIDs = [${knownDialectIDsAsConstArrayInner}] as const;
const knownFontIDs = [${knownFontIDsAsConstArrayInner}] as const;
const knownFontGroupIDs = [${knownFontGroupIDsAsConstArrayInner}] as const;
...
`.trim();
    ...
    const i18nFilePathname = path.join(GENERATED_SRC_DIR, I18N_TYPEDEF_FILENAME);

    const langOutFile = {
        filename: i18nFilePathname,
        contents: i18nFileContents,
        after: async () => console.info(`[INFO] Wrote out internationalization type definition file "${i18nFilePathname}"`),
    };
