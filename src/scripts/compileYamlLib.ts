...
import {getRecordEntries, getRecordValues, runningInJest} from '../client/utils';
...
const I18N_TYPEDEF_FILENAME = "i18nTokens.d.ts";
export function getTypeScriptHelperDefinitionsFromConfig(rfc: ReturnedFinalConfig): {filename: string, contents: string, after: () => Promise<void>}[] {
    ...
    const i18nTokens = defaultLangConfig.dialects[DEFINITIONS_DEFAULT_LANG]?.tokens;
...
export type I18NTokens = ${knownTokensAsTypeUnionInner};
`;
...
//]);
//export type I18NTokens = z.infer<typeof i18nTokensSchema>;
...
//    for (const outerConfig of getRecordValues(rfc.default.configs)) {
//        const {config, configType} = outerConfig;
//        defaultConfig[configType] = config;
//    }
    ...
    return [langOutFile, jsonOutFile];
}
