...
export function getTypeScriptHelperDefinitionsFromConfig(rfc: ReturnedFinalConfig): {filename: string, contents: string, after: () => Promise<void>}[] {
    ...
    const knownDialectNames = Object.keys(defaultLangConfig.dialects);
    //const knownDialectNamesAsTypeUnionInner = knownDialectNames.map((tok) => `"${tok}"`).join(" | ");
    const knownDialectNamesAsConstArrayInner = constArrInner(knownDialectNames);
